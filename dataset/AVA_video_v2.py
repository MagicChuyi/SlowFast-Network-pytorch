...
class AVA_video(Dataset):
    ...
    def __init__(self,data_dir,discard=True):
        ...
        path_to_AVA_dir = os.path.join(self.path_to_data_dir, 'ava','ava', 'preproc_train')
        ...
        # self.read_file_to_dic(path_to_video_ids_txt, self.detector_bbox_dic)
        # self.make_multi_lable(self.detector_bbox_dic)
    ...
    def __getitem__(self, index: int) -> Tuple[str, Tensor, Tensor, Tensor, Tensor]:
        ...
        #self.show_net_input(buffer,self.detector_bboxes_list[index],self.bboxes[index],self.labels[index],scale)
        ...
        #debug
        # for n,i in enumerate(self.labels[index]):
        #     print("bbox:",n)
        #     for j in i:
        #         print(self.i2c_dic[str(j)])




        labels = torch.tensor(one_hot_lable, dtype=torch.float)
        #print("one_hot_label:",labels)
        ...
        #print("bbox:", bboxes)
        #print("before_scale_detector_bboxes:", detector_bboxes)
        bboxes[:,[0,2]]*= scale[0]
        bboxes[:,[1,3]]*= scale[1]
        detector_bboxes[:,[0,2]]*= scale[0]
        detector_bboxes[:,[1,3]]*= scale[1]
        # print("dataloader_detector_bboxes:",detector_bboxes)
        # print("dataloader_scale:",scale)
        ...
        if len(buffer)<72:
            try:
                ...
                assert len(buffer)==72
                    #print("warning appen-1",len(buffer),fname)
            except:
...
if __name__ == '__main__':
    a=AVA_video('/home/aiuser/ava_v2.2/result.txt')
    a.imshow(10)
