...
class Base(torch.utils.data.dataset.Dataset):
    ...
    def padding_collate_fn(batch: List[Tuple[str, Tensor, Tensor, Tensor, Tensor]]) -> Tuple[List[str], Tensor, Tensor, Tensor, Tensor]:
        image_id_batch, image_batch, scale_batch, bboxes_batch, labels_batch,detector_bbox_batch,_ = zip(*batch)

        max_image_width = max([it.shape[3] for it in image_batch])
        max_image_height = max([it.shape[2] for it in image_batch])
        max_bboxes_length = max([len(it) for it in bboxes_batch])
        max_labels_length = max([len(it) for it in labels_batch])
        max_detector_length=max([len(it) for it in detector_bbox_batch])
        padded_image_batch = []
        padded_bboxes_batch = []
        padded_labels_batch = []
        padded_detector_batch=[]
        padded_image_batch=[]

        for i in image_batch:
            if np.array(i).shape[1]!=72:
                print(np.array(i))
            padded_image_batch.append(i)
        # for image in image_batch:
        #     #print('image', np.shape(image))
        #     padded_image = F.pad(input=image, pad=(0, max_image_width - image.shape[3], 0, max_image_height - image.shape[2]))  # pad has format (left, right, top, bottom)
        #     #print('delta',max_image_width - image.shape[3],max_image_height - image.shape[2])
        #     padded_image_batch.append(padded_image)
        #     #print('padded_image',np.shape(padded_image))
        ...
        for labels in labels_batch:
            ...
            if labels.shape[0]<max_labels_length:
                #print('###lables:', labels,labels.shape, max_labels_length)
                #padded_labels = torch.cat([labels, torch.zeros(max_labels_length - len(labels)).to(labels)])
                for i in range(max_labels_length-labels.shape[0]):
                    #print("concat:",max_labels_length-labels.shape[0],i)
                    temp=np.zeros((1,81))
                    #print(temp.shape)
                    #print(labels.shape)
        ...
        for detector_bboxes in detector_bbox_batch:
            padded_detector_bboxes = torch.cat([detector_bboxes, torch.zeros(max_detector_length - len(detector_bboxes), 4).to(detector_bboxes)])
            padded_detector_batch.append(padded_detector_bboxes)
        ...
        padded_detector_batch=torch.stack(padded_detector_batch,dim=0)
        padded_labels_batch = torch.stack(padded_labels_batch, dim=0)
        # print("padded_bboxes_batch",padded_bboxes_batch)
        # print("padded_labels_batch", padded_labels_batch)
        # print("padded_detector_batch",padded_detector_batch)
        #print("image_id_batch", image_id_batch)
        # print("scale_batch", scale_batch)
        return image_id_batch, padded_image_batch, scale_batch, padded_bboxes_batch, padded_labels_batch,padded_detector_batch
