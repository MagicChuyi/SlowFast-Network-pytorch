...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str]):
    ...
    if path_to_resuming_checkpoint is not None:
        ...
        Log.i('Model has been restored from file: {path_to_resuming_checkpoint}')
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            if step % num_steps_to_display == 0:
                ...
                print_loss='anchor_objectness_losses={ao:.6f}, anchor_transformer_losses={at:.6f}, proposal_class_losses={pc:.6f}, proposal_transformer_losses{pt:.6f}'\
                .format(ao=anchor_objectness_losses.item(), at=anchor_transformer_losses.item(), pc=proposal_class_losses.item(), pt=proposal_transformer_losses.item())
                print(print_loss)
                Log.i('anchor_objectness_losses={0}, anchor_transformer_losses={1}, proposal_class_losses={2}, proposal_transformer_losses{3}'.format(anchor_objectness_losses.item(), anchor_transformer_losses.item(), proposal_class_losses.item(), proposal_transformer_losses.item()))
            ...
            if step % num_steps_to_snapshot == 0 or should_stop:
                ...
                Log.i('Model has been saved to {0}'.format(path_to_checkpoint))
            if should_stop:
                break
    Log.i('Done')
...
if __name__ == '__main__':
    def main():
        ...
        parser.add_argument('-s', '--dataset', type=str, default='voc2007',choices=DatasetBase.OPTIONS, required=True, help='name of dataset')
        parser.add_argument('-b', '--backbone', type=str, default='resnet101',choices=BackboneBase.OPTIONS, required=True, help='name of backbone model')
