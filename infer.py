...
def _infer(path_to_input_image: str, path_to_output_image: str, path_to_checkpoint: str, dataset_name: str, backbone_name: str, prob_thresh: float):
    ...
    model = Model(backbone, dataset_class, pooler_mode=Config.POOLER_MODE,
                  anchor_ratios=Config.ANCHOR_RATIOS, anchor_sizes=Config.ANCHOR_SIZES,
                  rpn_pre_nms_top_n=Config.RPN_PRE_NMS_TOP_N, rpn_post_nms_top_n=Config.RPN_POST_NMS_TOP_N).cuda()
    ...
    with torch.no_grad():
        ...
        for bbox, cls, prob in zip(detection_bboxes.tolist(), detection_classes.tolist(), detection_probs.tolist()):
            ...
            draw.text((bbox.left, bbox.top), text='{category:s} {prob:.3f}', fill=color)

        image.save(path_to_output_image)
        print('Output image is saved to {path_to_output_image}')
